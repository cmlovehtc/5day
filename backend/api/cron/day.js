...
module.exports = async (req, res) => {
  ...
  for (const symbol of SYMBOLS) {
    try {
      ...
      results.push({ symbol, marketCode: 0, ok: true, fetchedAtTaipei: payload.fetchedAtTaipei });
    } catch (e) {
      results.push({ symbol, marketCode: 0, ok: false, error: String(e?.message || e) });
    }
  }
  return res.status(200).json({ ok: true, type: "day", results });
};
